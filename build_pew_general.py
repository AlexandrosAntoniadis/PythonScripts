...
if __name__ == '__main__':
    ...
    # Work on left side
    wavelength_left, flux_left = cut_data(wavelength, flux, w1, critical_wl)
    idx_left = np.argmax(flux_left)
    wl_max_flux_left = wavelength_left[idx_left]
    fl_max_flux_left = flux_left[idx_left]

    # Work on right side
    wavelength_right, flux_right = cut_data(wavelength, flux, critical_wl, w2)
    ...
    f = flux[idx]
    g = np.poly1d(g)(x)
    area = area_between(f, g, dx=x[1]-x[0])
    print(r'Area of line: {:.2f}mÅ'.format(area*1000))
