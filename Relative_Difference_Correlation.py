...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec 11 12:58:10 2017
...
def Get_Correlation(list1, list2, name1, name2):
    ...
    # set figure title
    plt.title(name1[7:]+" EW Relative Difference Correlation", fontsize=set_res*2)
    # set axis labels
    plt.xlabel("AA_EW_"+name1[7:], fontsize=set_res*1.5)
    plt.ylabel("% difference", fontsize=set_res*1.5)
    
    plt.plot(list1,((list1-list2)/list2*100), 'k.')
    ...
    #plt.text(np.min(list1), np.nanmin(list1-list2), "__  Mean difference = " + "{0:.5f}".format(dif_average)\
     #        + "\n" + "_._ Median difference = " + "{0:.5f}".format(dif_median)\
      #       + "\n" + "---  Standard deviation = " + "{0:.5f}".format(dif_std)\
       #      , color='black', horizontalalignment ='right', verticalalignment = 'top', bbox=dict(facecolor='none', edgecolor='black', boxstyle='round,pad=1'))
    ...
    plt.savefig('ew_rel_diff_cor_results/ew_rel_diff_corr_%s.png'%name1, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    if not os.path.exists('ew_rel_diff_cor_results'):
        os.makedirs('ew_rel_diff_cor_results')
